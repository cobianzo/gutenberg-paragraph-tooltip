...
import {
	addAttributeToCurrentLink,
	createTooltipInputElement,
	findPopupContainer,
	findCurrentLink,
	createTooltipLabelElement,
} from './helpers';
		...
		if ( popups.length > 0 ) {
			...
			const observer = new MutationObserver(
				( mutationsList: MutationRecord[] ) => {
					...
					const observedElement: HTMLElement | undefined = [
						...observersForPopups.keys(),
					...
					for ( const mutation of mutationsList ) {
						if ( mutation.type === 'childList' ) {
							...
							const container: NullableHTMLElement =
								findPopupContainer();
							if ( ! container ) {
								return;
							}

							const fieldSetOptionsContainer =
								container instanceof HTMLElement
									? container.querySelector(
										'fieldset.block-editor-link-control__settings'
									)
									: null;
							...
							if (
								container.querySelector( `input#${ newInputId }` )
							) {
								return;
							}
							if (
								fieldSetOptionsContainer.getAttribute(
									'data-observerTooltip'
								) === 'true'
							) {
								return;
							}
							fieldSetOptionsContainer.setAttribute(
								'data-observerTooltip',
								'true'
							);
							...
							setTimeout( () => {
								...
								if ( ! currentLinkText || ! currentLinkURL ) {
									...
									console.error(
										'No previous values for link url and content',
										currentLinkText,
										currentLinkURL
									);
									return;
								...
								const saveBtn: NullableHTMLElement =
									container?.querySelector( saveBtnSelector );

								const _label: HTMLElement =
									createTooltipLabelElement();
								const tooltipInput: HTMLInputElement =
									createTooltipInputElement(
										container,
										saveBtn
									);
								...
								fieldSetOptionsContainer.appendChild(
									tooltipInput
								);
